...
## run ss server
docker run --name=ss -p 4443:8388 -d docker.io/wwyyzz/shadowsocks /usr/bin/ss-server -s 0.0.0.0 -p 8388 -k 'PASS' -m rc4-md5 &&
...
docker run -it --rm --volumes-from ikev2-vpn -e "HOST=IP.106.88.48" docker.io/wwyyzz/ikev2-vpn  generate-mobileconfig > /root/ikev2-vpn.mobileconfig &&
